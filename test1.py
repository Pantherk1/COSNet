...
import cv2
import numpy as np
# from evaluation import calc_acc, calc_sen, calc_fdr, calc_spe, calc_kappa, calc_gmean, calc_iou, calc_dice, calc_auc
...
def test_first_stage1(dataloader, net, device, results_dir,
...
def test_first_stage(dataloader, net, device, results_dir,
    ...
    with torch.no_grad():
        for sample in dataloader:
            ...
            thick_gt = sample[3].to(device)
            outputs = net(img)
            thick_pred, thin_pred, pred=outputs['seg_thick'],outputs['seg_thin'],outputs['seg']
            if len(sample) == 5:
                w, h = sample[4]
