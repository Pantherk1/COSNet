...
from test1 import test_first_stage,test_first_stage1, test_second_stage
...
def val_first_stage(best_thin, best_thick, best_fusion,  writer, dataloader, net,
    ...
    thin_auc = round(visual_results(loss_dct["thin"], auc_dct["thin"], acc_dct["thin"], sen_dct["thin"], fdr_dct["thin"], spe_dct["thin"], kappa_dct["thin"], gmean_dct["thin"], iou_dct["thin"], dice_dct["thin"],
                                    writer, epoch, flag="_thin") + 1e-12, 4)
    ...
    writer.write('%.4f,%.4f,%.4f\n'%(thin_auc,thick_auc,fusion_auc))
...
def val_first_stage1(best_thin, best_thick, best_fusion,  writer, dataloader, net,
                    thin_criterion, thick_criterion, fusion_criterion, device, save_epoch_freq,
                    models_dir, results_dir, epoch, num_epochs=100):
    net.eval()
    loss_dct, auc_dct, acc_dct, sen_dct, fdr_dct, spe_dct, kappa_dct, gmean_dct, iou_dct, dice_dct \
    = test_first_stage1(dataloader, net, device, results_dir,
                       thin_criterion, thick_criterion, fusion_criterion, isSave=True)
    thin_auc = round(visual_results(loss_dct["thin"], auc_dct["thin"], acc_dct["thin"], sen_dct["thin"], fdr_dct["thin"], spe_dct["thin"], kappa_dct["thin"], gmean_dct["thin"], iou_dct["thin"], dice_dct["thin"],
                                    writer, epoch, flag="_thin") + 1e-12, 4)
    thick_auc = round(visual_results(loss_dct["thick"], auc_dct["thick"], acc_dct["thick"],
